...
export interface PreviewBinding {
  ...
  action: ActionName; intentHash: Hex; policyHash: Hex; evidenceHash: Hex; blockHash: Hex; expiresAt: string;
}
...
type Record = { intent: ActionIntent; binding: PreviewBinding; keyStateHash: Hex; blockNumber: bigint };
...
export class ActionService {
  ...
  async preview(input: ActionIntent, signal?: AbortSignal) {
    ...
    const expires = this.context.now() + ttl * 1000;
    ...
    if (risk.decision !== "accept") return result({ previewId: null, risk, transaction: undefined, analysisOnly: true }, null, risk.warnings);
  ...
  async prepare(previewId: string, signal?: AbortSignal) {
    ...
    return result(prepared, ctx.snapshot, [{ code: "WALLET_REQUIRED", severity: "info", message: "Unsigned payload only. External wallet signs and sends. Repeated preparation is not duplicate-transaction protection." }]);
  }
}
