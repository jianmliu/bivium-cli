...
export function previewMarketMaking(input: MarketMakingInput) {
  ...
  validateGroups(all);
  // Future-start signatures already authorize future inventory; conservatively include all unexpired windows.
  const live = all.filter((entry) => input.now <= entry.offer.expiry);
