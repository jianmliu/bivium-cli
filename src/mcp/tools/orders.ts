...
import { bytesSchema, hashSchema, objectSchema, offerSchema, reviveOffer, mcpOffer } from "./reads.ts";
...
export const ORDER_TOOL_SPECS: ToolDef[] = [
  ...
  { name: "order_delist", description: "Opt-in advisory relayer removal. The maker must EIP-191 personal_sign the exact UTF-8 message bivium-cancel:<commitment>, where <commitment> is the lowercase 0x-prefixed bytes32 commitment, with no whitespace or newline. Does not revoke signatures or cancel on chain.", inputSchema: objectSchema({ offer: offerSchema, commitment: hashSchema, cancelSignature: bytesSchema }), annotations: { readOnlyHint: false, destructiveHint: true, idempotentHint: true } },
];
export function createOrderTools(service: OrderService): ToolDefinition[] {
  return ORDER_TOOL_SPECS.map((tool) => ({ ...tool, concurrency: "managed", timeoutMs: SIMULATION_TIMEOUT_MS, handler: async (args, { signal }) => {
    switch (tool.name) {
      case "order_prepare": { const response = await service.prepare(args as unknown as OrderIntent, signal); return { ...response, data: { ...response.data, offer: mcpOffer(service.context, response.data.offer) } }; }
      case "order_publish": {
