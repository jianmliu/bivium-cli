...
export interface ToolDefinition extends ToolDef { handler(args: Record<string, unknown>, context: ToolContext): Promise<unknown> | unknown; timeoutMs?: number }
...
export class ToolRegistry {
 ...
 list(): ToolDef[] { return [...this.entries.values()].map(({definition: {handler,timeoutMs,...definition}}) => definition); }
 async call(name: string, args: unknown, context?: {signal?: AbortSignal}): Promise<unknown> {
  ...
  return withDeadline(signal => this.limit(() => entry.definition.handler(args as Record<string,unknown>,{signal}),signal),entry.definition.timeoutMs,context?.signal);
 }
}
