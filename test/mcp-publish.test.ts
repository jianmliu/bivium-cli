...
test('publication rechecks inventory limits and rejects changed inventory policy',async()=>{for(const change of ['balance','policy']){const dir=mkdtempSync(join(tmpdir(),'inventory-')),journal=new PublishJournal(dir),f=fixture();(f.actions.policies.test as any).inventory[intent.marketId].maxCredit='115';let sends=0;const service=new OrderService(f.context,f.actions,{journal,allowRelayerWrites:true,fetch:async()=>{sends++;return new Response('{}');}});try{const p=await service.prepare(intent);const signature=await maker.signTypedData(p.data.typedData!);if(change==='policy')(f.actions.policies.test as any).inventory[intent.marketId].maxCredit='116';else{const read=f.context.rpc.readContract;f.context.rpc.readContract=async r=>r.functionName==='creditOf'?120n:read(r);}await assert.rejects(service.publish({prepareId:p.data.prepareId,signature}),/POLICY_REJECTED|STATE_CHANGED/);assert.equal(sends,0);}finally{journal.close();rmSync(dir,{recursive:true,force:true});}}});
